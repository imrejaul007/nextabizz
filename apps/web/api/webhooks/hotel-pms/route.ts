 ...
 *
 * POST /api/webhooks/hotel-pms
 *
 * Receives inventory signal webhooks from Hotel PMS when hotel inventory
 * (housekeeping, kitchen, spa, etc.) drops below threshold.
 * Validates the HMAC signature, parses and validates the payload, then processes the signal.
 */

const WEBHOOK_SOURCE = 'hotel-pms' as const;
...
function validateEnvironment(): { valid: boolean; error?: string } {
  const secret = process.env.WEBHOOK_SECRET_HOTEL_PMS;
  ...
  if (!secret) {
    return {
      valid: false,
      error: 'WEBHOOK_SECRET_HOTEL_PMS environment variable is not set',
    };
  }
  ...
  try {
    ...
    try {
      const isValid = verifyWebhookSignature(rawBody, headers, {
        secret: process.env.WEBHOOK_SECRET_HOTEL_PMS!,
    ...
    let validatedPayload: ReturnType<typeof validateHotelPMSPayload>;
    try {
      validatedPayload = validateHotelPMSPayload(payload);
    ...
    // Process the webhook
    const result = await handleHotelPMSInventorySignal(validatedPayload, handlerContext);
